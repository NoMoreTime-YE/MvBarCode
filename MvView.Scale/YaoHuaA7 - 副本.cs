...
namespace MvView.Scale
{
    ...
    /// </summary>
    internal class YaoHuaA7 : IScale
    {  
        // 串口对象
        ...
        private void ProbeDev()
        {
            ...
            {  
                ...
                catch(Exception e)
                {
                    ...
                    Debug.WriteLine("Open serial " + port + " exception: ", e.Message);
                }
            }
        }
        ...
        // 是否开始取_WightCache中的重量至_SampleArray数组中
        private long _SampleFlag = 0;

        // 每组的采样个数
        private static int _nSampleCountPerGroup = 4;
        ...
        // 是否开始计算_SampleArray中的重量数据是否满足方差
        ...
        private Thread _GetBufferWeightThread;
            ...
            try
            {
                ...
                while (_bSerialLooping)
                {
                    ...
                    try
                    {
                        ...
                        double weight = ParseStrData(_Serial.ReadTo("="));

                        //if (SelectDataFlag % 2 != 0)
                        //{
                        //    if (SelectDataFlag >= 1)
                        //        SelectDataFlag = 0;
                        //    continue;
                        ...
                        MvBarCode.MvBarCodeGlobalVar.Log.DebugFormat("实时重量：{0}",weight);
                        // 保护flag
                        lock(_LockFlagObj)
                        { 
                            // 刷新缓冲
        ...
        public virtual bool Open(string info)
        {
            if (_Serial == null)
            {
                ...
                if (_Serial != null)
                {
                    ...
                    if (_GetBufferWeightThread==null)
                    {
                        _GetBufferWeightThread = new Thread(GetBufferWeight);
                        _GetBufferWeightThread.IsBackground = true;
                        _GetBufferWeightThread.Start();
                    }
